...
export default function MainPage() {
    ...
    const handleChangeAutoComplete = useCallback((value: string) => {
        if (value) {
    ...
    const handleChangeTextField = useCallback(async (value: string) => {
        if (value.includes("#")) {
            debounce(async () => {
                const [name, tag] = value.split("#");
        ...
        <>
            <MainWrapper>
                <MainContentWrapper>
                    ...
                    <AutoCompleteWrapper>
                        <CustomAutoComplete id="main_auto_complete" searchHistory={LS_SEARCH_HISTORY} options={userList} onInputChange={handleChangeTextField} onChange={handleChangeAutoComplete} style={{ width: "500px" }} />
                    </AutoCompleteWrapper>
                </MainContentWrapper>
            </MainWrapper>
        </>
