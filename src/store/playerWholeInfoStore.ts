...
export const latestFiveGamesStatsState = selector<LatestFiveGamesStatsState>({
    ...
    get: ({ get }) => {
        const LATEST_FIVE_GAMES: Array<MatchHistory> | undefined = get(latestFiveGamesState);
...
export const latestFiveGamesKDRatioState = selector<number>({
    ...
    get: ({ get }) => {
        const LATEST_FIVE_GAMES_STATS: LatestFiveGamesStatsState = get(latestFiveGamesStatsState);
...
export const seasonGameWinDefeatRecordState = selector<PrevSeason>({
    ...
    get: ({ get }) => {
        ...
        const result = Array.from(Object.values(BY_SEASON_DATA)).reduce((PREV_SEASON: PrevSeason, CURR_SEASON: SeasonData) => {
...
export const latestFiveGamesDPRState = selector<number>({
    ...
    get: ({ get }) => {
        ...
        return Number((latestFiveGamesWholeDamage / latestFiveGamesRounds).toFixed(2));
    }
...
export const agentPlayInfosState = selector({
    ...
    get: ({ get }) => {
        ...
        let UNFILTERED_INFOS: Array<UnFilteredAgentPlayInfoState> = LATEST_FIVE_GAMES.reduce((prev, curr) => {
            const PLAYER: Player = curr.players.all_players.filter(player => player.puuid === get(puuidState))[0];
        ...
        const FILTERED_INFOS = UNFILTERED_INFOS.reduce((prevInfo: PrevInfo, curr: UnFilteredAgentPlayInfoState) => {
            ...
            if (agent in prevInfo) {
                return {
                    ...
                    [agent]: {
                        ...
                        avgRecord: Number((((prevInfo[agent].kills + kills) + (prevInfo[agent].assists + assists)) / (prevInfo[agent].deaths + deaths)).toFixed(2)),
                        avgScore: Number(((prevInfo[agent].score + score) / (prevInfo[agent].matchCount + 1)).toFixed(0)),
                        winRatio: Number(((prevInfo[agent].matchWins + (match_result === "WIN" ? 1 : 0)) / (prevInfo[agent].matchCount + 1) * 100).toFixed(0)),
                        matchWins: prevInfo[agent].matchWins + (match_result === "WIN" ? 1 : 0),
...
export const weaponStatsInfoState = selector({
    ...
    get: ({ get }) => {
        ...
        const WHOLE_KILLS = LATEST_FIVE_GAMES.reduce((PREV_MATCH, CURRENT_MATCH) => {
        ...
        const KILL_INFO_GROUP_BY_WEAPON = WHOLE_KILLS.reduce((prevWeapon: PrevWeapon, curr: Kills) => {
...
export const matchHistoryGroupByMapState = selector({
    ...
    get: ({ get }) => {
        ...
        const MatchHistoryGroupByMap = LATEST_FIVE_GAMES.reduce((prev: PrevMap, curr: MatchHistory) => {
            ...
            if (prev[MAP]) {
                return {
                    ...
                    [MAP]: {
                        ...
                        winRatio: Number(((prev[MAP].matchWins + MATCH_RESULT) / (prev[MAP].matchCount + 1) * 100).toFixed(0)),
...
export const agentInfoState = selector({
    ...
    get: ({ get }) => {
        ...
        let UNFILTERED_INFOS: Array<UnFilteredInfos> = LATEST_FIVE_GAMES.reduce((prev: Array<UnFilteredInfos>, curr: MatchHistory) => {
            const PLAYER = curr.players.all_players.filter(player => player.puuid === get(puuidState))[0];
            type Team = keyof typeof curr.teams;
            const TEAM = PLAYER.team.toLowerCase() as Team;
            const MATCH_RESULT = curr.teams?.[TEAM].has_won ? 'WIN' : 'DEFEAT';
        ...
        const FILTERED_INFOS: PrevInfo = UNFILTERED_INFOS.reduce((prevInfo: PrevInfo, curr) => {
            const { agent, assists, deaths, kills, match_result, score, AGENT_ICON_URL } = curr;
